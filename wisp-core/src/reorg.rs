...
impl Blockchain {
    ...
    pub fn reorganize_chain(
            ...
            .transaction(
                |tx_db| -> Result<(), ConflictableTransactionError<ReorgError>> {
                    ...
                    for block_to_apply in &new_chain_segment {
                        ...
                        block_to_apply.validate_block_for_reorg(&temp_utxos.utxos, &expected_target, tx_db, &self.db).map_err(ReorgError::Anyhow)?;

                        // Apply the block to the database using the shared helper.
                        self.apply_block_to_db(tx_db, block_to_apply, &new_chain_segment, &mut new_supply, &mut new_tx_count)?;
        ...
        if let Some(bytes) = self.db.get(DBKeys::PENDING_UTXO_SNAPSHOT)? {
            ...
            // Promote the pending snapshot to the main snapshot to be used as the next recovery baseline.
